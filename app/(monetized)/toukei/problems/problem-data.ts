...
export const problems: Problem[] = [
  ...
  },
  {
    ...
    title: "2つの母比率の差の検定（A/Bテストにおけるプールされた比率とz検定）",
    description: "独立した2群の標本比率の差を比較する際、帰無仮説の下で母比率が等しいと仮定して算出する「プールされた比率」とz検定統計量の導出手順を習得します。",
    concepts: ["母比率の差の検定", "プールされた比率", "z検定", "大標本近似", "A/Bテスト"],
    question: "あるECサイトで、購入完了画面のデザイン改善効果を検証するためA/Bテストを実施した。利用者をデザインA・Bへ無作為に割り付け、各利用者は一方だけを閲覧し、購入結果は互いに独立とする。従来デザインAを訪問した利用者500名のうち40名が購入し、新デザインBを訪問した利用者500名のうち65名が購入した。\n帰無仮説 H_0: 「新旧デザインの母購入比率は等しい（p_A = p_B）」、対立仮説 H_1: 「新旧デザインの母購入比率は異なる（p_A ≠ p_B）」として、有意水準 α = 0.05 の両側z検定を行う。\n(1) 帰無仮説の下でのプールされた標本比率 p̂ を求めなさい。\n(2) 検定統計量 z の値（小数第2位まで）を求め、有意水準5%における検定結果を判定しなさい。ただし、標準正規分布の両側5%点（上側2.5%点）の臨界値は z_0.025 = 1.96 とし、大標本による正規近似が成り立つものとする。",
    givenValues: [
      { label: "デザインA 標本サイズ (n_A)", value: "500" },
      { label: "デザインA 購入者数 (x_A)", value: "40（標本比率 p̂_A = 0.080）" },
      { label: "デザインB 標本サイズ (n_B)", value: "500" },
      { label: "デザインB 購入者数 (x_B)", value: "65（標本比率 p̂_B = 0.130）" },
      { label: "有意水準 (α)", value: "0.05（両側検定）" },
      { label: "正規分布の臨界値 (z_0.025)", value: "1.96" }
    ],
    solutionSteps: [
      {
        label: "プールされた標本比率 p̂ の計算",
        expression: "p̂ = (x_A + x_B) / (n_A + n_B) = (40 + 65) / (500 + 500) = 105 / 1000 = 0.105",
        description: "帰無仮説 H_0: p_A = p_B の下では、両群は同一の共通母比率 p を持つと想定されるため、2群のデータを合算して最も精度の高い共通母比率の推定量（プールされた比率）を算出します。1 - p̂ = 0.895 となります。"
      },
      {
        label: "標本比率の差の標準誤差 SE の計算",
        expression: "SE = √(p̂(1 - p̂)(1/n_A + 1/n_B)) = √(0.105 * 0.895 * (1/500 + 1/500)) = √(0.0003759) ≒ 0.019388",
        description: "帰無仮説が正しい場合の標本比率の差 (p̂_B - p̂_A) の分散は p̂(1 - p̂)(1/n_A + 1/n_B) で与えられます。平方根をとると約0.01939となります。"
      },
      {
        label: "検定統計量 z の算出と仮説検定の判定",
        expression: "z = (p̂_B - p̂_A) / SE = (0.130 - 0.080) / 0.019388 = 0.050 / 0.019388 ≒ 2.58",
        description: "算出した検定統計量 z = 2.58 は、両側検定の臨界値 1.96 を上回っています（|z| = 2.58 > 1.96）。p値は約0.0099（p < 0.01）であり、有意水準5%（さらには1%水準でも）において帰無仮説は棄却されます。したがって、新デザインBの購入比率は従来デザインAと有意に差がある（向上している）と判断されます。"
      }
    ],
    finalAnswer: "(1) プールされた標本比率は p̂ = 0.105、(2) 検定統計量は z ≒ 2.58 であり、臨界値 1.96 より大きいため有意水準5%で帰無仮説は棄却される（新旧デザインの購入率に有意な差が認められる）。",
    distractors: [
      {
        value: "p̂ = 0.105, z = 2.59（プールせず各群の比率で分散を別個に計算した場合）",
        reason: "各群の標本比率を個別に使った標準誤差 √(p̂_A(1-p̂_A)/n_A + p̂_B(1-p̂_B)/n_B) ≒ 0.01932 を用いた場合の計算値（z ≒ 2.59）です。これは母比率の差の信頼区間を構成する際の公式であり、帰無仮説 H_0: p_A = p_B の検定ではプールされた比率を用いるのが標準的な統計手順です。"
      },
      {
        value: "p̂ = 0.105, z = 1.33（分母のサンプルサイズを足して n=1000 で割る誤り）",
        reason: "分散の計算で (1/n_A + 1/n_B) の代わりに 1/(n_A + n_B) = 1/1000 を用いてしまう典型的な誤りです。"
      },
      {
        value: "p̂ = 0.210（各群の比率 0.08 + 0.13 を単純合計する誤り）",
        reason: "プール比率は加重平均（合算比率）であり、各群の比率を足し合わせるものではありません。"
      }
    ],
    relatedGuideSlugs: ["hypothesis-testing-basics", "choosing-statistical-tests"],
    appLinks: [
      { title: "分野別ドリルで「仮説検定」を演習する", url: "https://bearworks.uk/toukei/drill" },
      { title: "チートシートで母比率の検定統計量を確認する", url: "https://bearworks.uk/toukei/cheatsheet" }
    ],
    author: "kuma / bearworks.uk",
    publishedAt: "2026-09-05",
    reviewedAt: "2026-09-05",
    provenance: toukeiProblemBatch5Provenance,
    references: [
      { title: "統計検定2級公式出題範囲", url: "https://www.toukei-kentei.jp/grade/grade2/" },
      { title: "NIST：Two Process Proportions", url: "https://www.itl.nist.gov/div898/handbook/prc/section3/prc33.htm" }
    ]
  },
  {
    slug: "exponential-distribution-waiting-time",
    title: "指数分布（ポアソン過程における事象発生間隔と無記憶性）",
    description: "単位時間あたり一定の平均発生率を持つポアソン過程において、次の事象が発生するまでの待ち時間をモデル化する指数分布の確率計算、期待値、および無記憶性を理解します。",
    concepts: ["指数分布", "ポアソン過程", "無記憶性", "ハザード率", "連続型確率分布"],
    question: "あるコールセンターでは、顧客からの電話着信が平均して1時間あたり λ = 6.0 回のポアソン過程に従ってランダムに発生している。次の着信があるまでの待ち時間を X（単位：時間）とする。\n(1) 次の電話着信があるまでの平均待ち時間（期待値、単位：分）を求めなさい。\n(2) ある通話が終了してから、次の着信があるまでに15分（0.25時間）以上待たされる確率 P(X ≥ 0.25) を求めなさい。ただし、e^(-1.5) ≒ 0.2231 とする。\n(3) すでに10分間着信を待っているとき、そこからさらに15分以上待たされる条件付き確率を求め、指数分布の「無記憶性」について簡潔に説明しなさい。",
    givenValues: [
      { label: "単位時間あたりの平均発生率 (λ)", value: "6.0 回/時間" },
      { label: "評価対象の待ち時間 (t)", value: "15分（0.25時間）" },
      { label: "指数値 e^(-1.5)", value: "0.2231" }
    ],
    solutionSteps: [
      {
        label: "平均待ち時間（期待値）の算出",
        expression: "E(X) = 1 / λ = 1 / 6.0 時間 = 10.0 分",
        description: "パラメータ λ の指数分布の期待値は E(X) = 1/λ です。1時間に6回発生するため、平均発生間隔は 1/6 時間、すなわち 60 / 6 = 10.0 分となります。"
      },
      {
        label: "15分以上待たされる確率 P(X ≥ 0.25) の算出",
        expression: "P(X ≥ t) = ∫[t, ∞] λ e^(-λ x) dx = e^(-λ t) = e^(-6.0 * 0.25) = e^(-1.5) ≒ 0.2231",
        description: "指数分布の生存関数（累積分布の右裾確率）は P(X ≥ t) = e^(-λ t) です。15分（0.25時間）以上発生しない確率は e^(-1.5) ≒ 0.2231（約22.3%）となります。"
      },
      {
        label: "無記憶性（Memoryless Property）の確認と解釈",
        expression: "P(X ≥ s + t | X ≥ s) = P(X ≥ s + t) / P(X ≥ s) = e^(-λ(s + t)) / e^(-λ s) = e^(-λ t) = P(X ≥ t) = e^(-1.5) ≒ 0.2231",
        description: "すでに s = 10分待ったという条件の下でさらに t = 15分以上待つ条件付き確率は、初めから15分以上待つ確率 e^(-1.5) ≒ 0.2231 と完全に一致します。過去にどれだけ待ったかという履歴は将来の待ち時間分布に一切影響を与えません。この性質を「無記憶性」と呼びます。"
      }
    ],
    finalAnswer: "(1) 平均待ち時間は 10.0分、(2) 15分以上待たされる確率は 約0.2231（約22.3%）、(3) すでに10分待った後にさらに15分待つ確率は 同様に約0.2231（約22.3%）である（過去の経過時間は将来の発生確率に影響しない無記憶性が成立する）。",
    distractors: [
      {
        value: "平均待ち時間: 6.0分, P(X ≥ 0.25) = 0.2231",
        reason: "パラメータ λ = 6.0 をそのまま平均待ち時間（6分）と誤認する誤りです。平均発生間隔は逆数 1/λ = 1/6 時間 = 10分です。"
      },
      {
        value: "P(X ≥ 0.25) = 0.7769",
        reason: "累積分布関数 F(t) = P(X ≤ 0.25) = 1 - e^(-1.5) ≒ 0.7769（15分以内に着信がある確率）を計算してしまう誤りです。15分以上待つ確率はその補事象 e^(-1.5) ≒ 0.2231 です。"
      },
      {
        value: "条件付き確率: 0.2231 より小さくなる（待った分だけすぐ来るはずという解釈）",
        reason: "ポアソン過程における無記憶性を理解せず、日常的な感覚（そろそろ電話が来るはず）に引きずられる誤謬です。機械の摩耗がない偶発故障やランダム着信では待ち時間はリセットされ続けます。"
      }
    ],
    relatedGuideSlugs: ["distribution-selection", "learning-roadmap"],
    appLinks: [
      { title: "分野別ドリルで「連続型確率分布」を練習する", url: "https://bearworks.uk/toukei/drill" },
      { title: "チートシートで確率分布の期待値・分散を比較する", url: "https://bearworks.uk/toukei/cheatsheet" }
    ],
    author: "kuma / bearworks.uk",
    publishedAt: "2026-09-05",
    ...
    references: [
      { title: "統計検定2級公式出題範囲", url: "https://www.toukei-kentei.jp/grade/grade2/" },
      { title: "NIST：Exponential Distribution", url: "https://www.itl.nist.gov/div898/handbook/eda/section3/eda3667.htm" }
    ]
  },
  {
    slug: "time-series-moving-average-autocorrelation",
    title: "時系列データの分析（中心化移動平均によるトレンド平滑化とラグ1の自己相関）",
    description: "時系列データの変動成分を平滑化する3項移動平均の計算と、直前時点のデータとの関連性を表すラグ1の自己相関係数の算出手順を習得します。",
    concepts: ["時系列分析", "移動平均", "自己相関係数", "ラグ", "平滑化", "定常性"],
    question: "ある新商品の発売直後6日間の日次販売数量 y_t（t = 1, 2, ..., 6）が記録された。\ny = [30, 42, 48, 36, 54, 60]（単位：個）\n6日間の標本平均は ȳ = 45.0 個、全偏差平方和は Σ_{t=1}^6 (y_t - ȳ)^2 = 630.0 である。\n(1) 奇数項（k=3）の移動平均法を用いて、t=2日目およびt=3日目における3日移動平均値をそれぞれ求めなさい。\n(2) この時系列データのラグ1（1期前）の自己相関係数 r_1 を求めなさい。ただし、自己相関係数の算出には全期間の標本平均 ȳ = 45.0 を用いる標準的な定義式\nr_1 = Σ_{t=1}^5 (y_t - ȳ)(y_{t+1} - ȳ) / Σ_{t=1}^6 (y_t - ȳ)^2\nを使用しなさい。",
    givenValues: [
      { label: "日次データ (t=1..6)", value: "30, 42, 48, 36, 54, 60" },
      { label: "標本平均 (ȳ)", value: "45.0 個" },
      { label: "全偏差平方和 (Σ(y_t - ȳ)²)", value: "630.0" },
      { label: "移動平均の項数", value: "3日（前後1日を含む中心化移動平均）" }
    ],
    solutionSteps: [
      {
        label: "3日移動平均値の算出",
        expression: "MA_2 = (30 + 42 + 48) / 3 = 120 / 3 = 40.0, MA_3 = (42 + 48 + 36) / 3 = 126 / 3 = 42.0",
        description: "奇数項の移動平均では、対象日を中心として前後対称に平均をとります。t=2日目の移動平均は (y_1 + y_2 + y_3)/3 = 40.0、t=3日目の移動平均は (y_2 + y_3 + y_4)/3 = 42.0 となります。短期間の不規則変動が平滑化され、緩やかな上昇トレンドが観察されます。"
      },
      {
        label: "各時点の偏差とラグ1の積和の計算",
        expression: "偏差 dev = [-15, -3, +3, -9, +9, +15]",
        description: "各時点の平均 ȳ = 45.0 からの偏差を求めます。y_1 - 45 = -15, y_2 - 45 = -3, y_3 - 45 = 3, y_4 - 45 = -9, y_5 - 45 = 9, y_6 - 45 = 15 です。"
      },
      {
        label: "ラグ1自己共分散分子の合算と自己相関係数 r_1 の算出",
        expression: "分子 = (-15)(-3) + (-3)(3) + (3)(-9) + (-9)(9) + (9)(15) = 45 - 9 - 27 - 81 + 135 = 63.0",
        description: "ラグ1の隣接積の総和は 63.0 となります。全偏差平方和 630.0 で割ると、自己相関係数は r_1 = 63.0 / 630.0 = 0.100（+0.10）となります。この標本で計算されたラグ1自己相関は+0.10です（なお、6観測の小標本のみから母過程の系列相関の有無を一般化して判断することはできません）。"
      }
    ],
    finalAnswer: "(1) 3日移動平均値は t=2 が 40.0個、t=3 が 42.0個である。(2) ラグ1の自己相関係数は r_1 = 0.100（+0.10）である。",
    distractors: [
      {
        value: "MA_2 = 40.0, MA_3 = 42.0, r_1 = 0.156",
        reason: "分母の偏差平方和を6項の合計 630.0 ではなく、ラグ1の積和に合わせた5項のみの平方和 405.0（225 + 9 + 9 + 81 + 81 = 405.0）を用いて割ってしまう誤りです（63.0 / 405.0 ≒ 0.156）。標準的な時系列定義では全期間の標本分散を分母に用います。"
      },
      {
        value: "MA_2 = 36.0 (t=1とt=2の2日平均をとる誤り)",
        reason: "3日移動平均の定義を誤り、過去2日間の平均等と取り違えてしまう誤りです。"
      },
      {
        value: "r_1 = -0.100 (符号の計算ミス)",
        reason: "積和の計算過程で負の積の符号を取り違えて合算してしまう誤りです。"
      }
    ],
    relatedGuideSlugs: ["learning-roadmap", "regression-interpretation"],
    appLinks: [
      { title: "分野別ドリルで「データの要約と時系列」を演習する", url: "https://bearworks.uk/toukei/drill" },
      { title: "学習ガイドで統計ロードマップを確認する", url: "https://bearworks.uk/toukei/guides/learning-roadmap" }
    ],
    author: "kuma / bearworks.uk",
    publishedAt: "2026-09-05",
    reviewedAt: "2026-09-05",
    ...
    references: [
      { title: "統計検定2級公式出題範囲", url: "https://www.toukei-kentei.jp/grade/grade2/" },
      { title: "NIST：Autocorrelation", url: "https://www.itl.nist.gov/div898/handbook/eda/section3/eda35c.htm" }
    ]
  },
  {
    slug: "two-way-anova-interaction",
    title: "二元配置分散分析（主効果の検定と交互作用効果の解釈）",
    description: "2つの要因を取り入れた実験計画において、分散分析表（ANOVA表）の自由度・平均平方・F比を正しく算出し、交互作用効果の有無とその統計的意味を判断します。",
    concepts: ["二元配置分散分析", "交互作用効果", "主効果", "分散分析表", "F検定", "実験計画法"],
    question: "ある教育研修機関で、新しい研修教材（要因A：従来版、新型版の2水準）と研修時間（要因B：短期集中、標準期間の2水準）が研修生の修了テスト得点に与える影響を調べるため、2×2要因の二元配置実験を実施した。各処理水準の組み合わせ（セル）ごとに5名ずつ、合計 N = 20 名の受講生を無作為に割り当てた（反復数 r = 5 の釣合い型実験）。各セルの誤差は互いに独立な正規分布に従い、分散は等しい（等分散性）と仮定する。\n得られたテスト結果から分散分析（ANOVA）を行ったところ、以下の平方和（SS）が得られた：\n・教材の主効果の平方和: SS_A = 180.0\n・時間の主効果の平方和: SS_B = 300.0\n・交互作用の平方和: SS_AB = 90.0\n・誤差（残差）の平方和: SS_E = 240.0\n(1) 交互作用効果 A×B に対する分散分析表の「自由度」「平均平方（MS_AB）」「F値（F_AB）」を求めなさい。\n(2) 自由度 (1, 16) のF分布における有意水準5%の臨界値は F_0.05(1, 16) ≒ 4.49 である。交互作用効果の検定結果と、交互作用が有意である場合の教育的・統計的解釈として最も適切なものを述べなさい。",
    givenValues: [
      { label: "要因A (教材) 水準数", value: "a = 2" },
      { label: "要因B (時間) 水準数", value: "b = 2" },
      { label: "各セルの反復数", value: "r = 5（総標本数 N = 20）" },
      { label: "各平方和 (SS)", value: "SS_A = 180.0, SS_B = 300.0, SS_AB = 90.0, SS_E = 240.0" },
      { label: "F分布の臨界値 F_0.05(1, 16)", value: "4.49" }
    ],
    solutionSteps: [
      {
        label: "各要因および誤差の自由度（df）の算出",
        expression: "df_A = a - 1 = 1, df_B = b - 1 = 1, df_AB = (a - 1)(b - 1) = 1, df_E = a * b * (r - 1) = 2 * 2 * 4 = 16, df_T = N - 1 = 19",
        description: "交互作用の自由度は (2 - 1) * (2 - 1) = 1 です。誤差（セル内残差）の自由度は 各セル内の自由度 (r - 1) = 4 を全4セルで足し合わせた 16 となります。"
      },
      {
        label: "平均平方（MS）と F値 の計算",
        expression: "MS_AB = SS_AB / df_AB = 90.0 / 1 = 90.0, MS_E = SS_E / df_E = 240.0 / 16 = 15.0, F_AB = MS_AB / MS_E = 90.0 / 15.0 = 6.00",
        description: "各平方和を自由度で除して平均平方（MS）を求めます（なお誤差平均平方 MS_E は共通誤差分散の推定量に相当します）。交互作用の検定統計量は F_AB = MS_AB / MS_E = 90.0 / 15.0 = 6.00 です。"
      },
      {
        label: "仮説検定の判定と交互作用の解釈",
        expression: "F_AB = 6.00 > 4.49 (臨界値) より 有意水準5%で有意（p ≒ 0.026）",
        description: "検定統計量 6.00 は臨界値 4.49 を上回るため、交互作用は統計的に有意です。交互作用が有意であるということは、「教材Aの効果の現れ方が、研修時間Bの水準によって異なる（例えば、新型教材は標準期間で実施して初めて大きな効果を発揮する等、要因間の相乗効果や条件依存性がある）」ことを意味します。この場合、教材や時間それぞれの主効果のみを単純に個別評価することはできず、水準の組み合わせごとに効果を検討・解釈する必要があります。"
      }
    ],
    solutionTable: {
      caption: "2×2 二元配置分散分析表（ANOVA表）",
      columns: ["要因（Source）", "平方和 (SS)", "自由度 (df)", "平均平方 (MS)", "F値", "判定 (α=0.05)"],
      rows: [
        ["要因A（教材）", "180.0", "1", "180.0", "12.00", "有意 (p < 0.01)"],
        ["要因B（時間）", "300.0", "1", "300.0", "20.00", "有意 (p < 0.001)"],
        ["交互作用 A×B", "90.0", "1", "90.0", "6.00", "有意 (p = 0.026)"],
        ["誤差（残差）", "240.0", "16", "15.0", "-", "-"],
        ["合計（全体）", "810.0", "19", "-", "-", "-"]
      ]
    },
    finalAnswer: "(1) 交互作用 A×B の自由度は 1、平均平方は 90.0、F値は 6.00 である。(2) F値 6.00 > 臨界値 4.49 より有意水準5%で交互作用は有意である。交互作用が有意な場合、教材の効果は研修時間によって異なるため、主効果を単独で評価するのではなく、両要因の組み合わせごとに効果を考察する必要がある。",
    distractors: [
      {
        value: "自由度 df_AB = 4, F_AB = 1.50",
        reason: "交互作用の自由度を水準数の積 a * b = 4 と誤認し、平均平方を 90 / 4 = 22.5 として F値を計算してしまう典型的な誤りです。自由度は (a - 1)(b - 1) = 1 です。"
      },
      {
        value: "誤差自由度 df_E = 17 (N - a - b = 20 - 2 - 2 = 16 ではなく全自由度から引く計算ミス)",
        reason: "誤差自由度の算出で要因と交互作用を混同する誤りです。df_E = N - a*b = 20 - 4 = 16 です。"
      },
      {
        value: "交互作用が有意であるため、教材の主効果のみを取り出して『どんな時間設定でも新型教材が常に優れている』と一般化して結論づける",
        reason: "交互作用が有意な場合の解釈として重大な誤りです。交互作用がある場合、一方の要因の効果は他方の要因の水準に依存するため、主効果を無条件に一般化することはできません。"
      }
    ],
    relatedGuideSlugs: ["anova-and-chi-square", "choosing-statistical-tests"],
    appLinks: [
      { title: "分野別ドリルで「分散分析」を演習する", url: "https://bearworks.uk/toukei/drill" },
      { title: "学習ガイドで分散分析の手法を確認する", url: "https://bearworks.uk/toukei/guides/anova-and-chi-square" }
    ],
    author: "kuma / bearworks.uk",
    publishedAt: "2026-09-05",
    reviewedAt: "2026-09-05",
    ...
    references: [
      { title: "統計検定2級公式出題範囲", url: "https://www.toukei-kentei.jp/grade/grade2/" },
      { title: "NIST：Models and calculations for the two-way ANOVA", url: "https://www.itl.nist.gov/div898/handbook/prc/section4/prc438.htm" }
    ]
  },
  {
    slug: "multiple-regression-multicollinearity-dummy",
    title: "重回帰分析の診断（多重共線性・VIFの算出とダミー変数の設定）",
    description: "重回帰モデルにおける説明変数間の強い相関による多重共線性の影響とVIF（分散拡大係数）の算出、および質的変数をモデルに組み込むためのダミー変数の設計原則（ダミー変数の罠）を理解します。",
    concepts: ["重回帰分析", "多重共線性", "VIF", "分散拡大係数", "ダミー変数", "ダミー変数の罠"],
    question: "ある不動産アナリストがマンションの家賃 y を予測するため、複数の説明変数を用いた重回帰分析を検討している。\n(1) 説明変数として「最寄り駅からの徒歩分数 X_1」と「主要ターミナル駅までの乗車時間 X_2」をモデルに投入したところ、両変数間の相関係数が r = 0.80 と高かった。このとき、X_1 を X_2 に単回帰した際の決定係数は R_1^2 = r^2 = 0.64 である。多重共線性の診断指標である分散拡大係数（VIF: Variance Inflation Factor）を算出しなさい（小数第2位まで）。また、無相関（VIF=1）の場合と比較して偏回帰係数の推定値の標準誤差が約何倍に拡大するか求めなさい（√2.78 ≒ 1.67 とする）。\n(2) さらに立地条件を表す質的変数「エリア区分」（都心部、準都心部、郊外 の3カテゴリー）を重回帰モデルに説明変数として導入したい。定数項（切片）を含む回帰モデルを構築する場合、多重共線性の発生（ダミー変数の罠）を避けるために投入すべき独立なダミー変数の個数と、その設定方法について最も適切な説明を選びなさい。",
    givenValues: [
      { label: "2つの説明変数間の相関係数 (r)", value: "0.80" },
      { label: "決定係数 (R_1²)", value: "0.64 (0.80²)" },
      { label: "VIF の平方根 (√2.78)", value: "1.67" },
      { label: "質的変数のカテゴリー数 (k)", value: "3（都心部、準都心部、郊外）" }
    ],
    solutionSteps: [
      {
        label: "分散拡大係数（VIF）の算出",
        expression: "VIF = 1 / (1 - R_1^2) = 1 / (1 - 0.64) = 1 / 0.36 = 100 / 36 = 25 / 9 ≒ 2.78",
        description: "VIFは他の説明変数との相関によって偏回帰係数の分散が何倍に拡大するかを示す指標です。決定係数が 0.64 のとき、VIF ≒ 2.78 となります。一般にVIFが10以上（研究分野によっては5以上）で深刻な多重共線性があると判断されます。"
      },
      {
        label: "偏回帰係数の標準誤差への影響の評価",
        expression: "標準誤差の拡大倍率 = √VIF = √2.78 ≒ 1.67 倍",
        description: "偏回帰係数の標準誤差は √VIF に比例して増大します。2変数間の相関が 0.80 あることにより、推定量の標準誤差は無相関の場合に比べて約1.67倍に膨らみ、t値が小さくなって真に効果がある変数が統計的有意となりにくくなります。"
      },
      {
        label: "ダミー変数の設定と「ダミー変数の罠」の回避",
        expression: "必要なダミー変数数 = k - 1 = 3 - 1 = 2 個",
        description: "カテゴリー数が k の質的変数を定数項のある回帰モデルに投入する場合、独立なダミー変数は k - 1 個設定します。例えば「郊外」を参照カテゴリー（基準）とし、D_1（都心なら1、他は0）および D_2（準都心なら1、他は0）の2つを投入します。仮に3つすべてのダミー変数を定数項とともに投入すると、D_1 + D_2 + D_3 = 1（定数項）という完全な線形従属（完全多重共線性）が生じ、推計パラメータが一意に定まらなくなります。これを「ダミー変数の罠（Dummy Variable Trap）」と呼びます。"
      }
    ],
    finalAnswer: "(1) 分散拡大係数は VIF ≒ 2.78 であり、標準誤差は約1.67倍に拡大する。(2) 定数項を含むモデルでは「ダミー変数の罠」を回避するためカテゴリー数より1つ少ない k - 1 = 2個のダミー変数を投入し、残る1つ（例：郊外）を参照カテゴリーとして係数を相対的に解釈する設計が最も適切である。",
    distractors: [
      {
        value: "VIF = 5.00 (相関係数 r = 0.80 を用いて 1 / (1 - 0.80) と計算する誤り)",
        reason: "分母で 1 - R²（決定係数）ではなく 1 - r（相関係数）を直接差し引いてしまう典型的な計算ミスです。"
      },
      {
        value: "ダミー変数は3カテゴリー全てに対応して3個投入し、定数項もそのまま含めるのが最も適切である",
        reason: "3個すべてのダミー変数と定数項を同時に投入すると、ダミーの和が常に1となって定数項と完全な線形関係が生じる『ダミー変数の罠』に陥り、通常の最小二乗法では逆行列が存在せず回帰係数が推定不能になります。"
      },
      {
        value: "VIF = 2.78 であるため、直ちに多重共線性が極めて深刻であり変数を必ず削除しなければならない",
        reason: "一般的に多重共線性が危険視される目安は VIF > 10（または厳格な基準で VIF > 5）であり、VIF ≒ 2.78 は注意は要するものの直ちに変数を削除すべき水準ではありません。"
      }
    ],
    relatedGuideSlugs: ["regression-interpretation", "choosing-statistical-tests"],
    appLinks: [
      { title: "分野別ドリルで「相関と回帰分析」を演習する", url: "https://bearworks.uk/toukei/drill" },
      { title: "学習ガイドで回帰分析のモデル診断を確認する", url: "https://bearworks.uk/toukei/guides/regression-interpretation" }
    ],
    author: "kuma / bearworks.uk",
    publishedAt: "2026-09-05",
    reviewedAt: "2026-09-05",
    ...
    references: [
      { title: "統計検定2級公式出題範囲", url: "https://www.toukei-kentei.jp/grade/grade2/" },
      { title: "Penn State STAT 462：Detecting Multicollinearity Using Variance Inflation Factors", url: "https://online.stat.psu.edu/stat462/node/180/" },
      { title: "scikit-learn：Avoiding Multicollinearity with One-Hot Dummy Encoding", url: "https://scikit-learn.org/stable/modules/generated/sklearn.preprocessing.OneHotEncoder.html" }
    ]
  }
];
