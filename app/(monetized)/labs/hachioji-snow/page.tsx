  ...
  return <main className="mx-auto w-full min-w-0 max-w-6xl px-4 py-6 text-slate-700 sm:px-6">
    ...
    <article className="space-y-10 leading-relaxed">
      ...
      <section className={card} id="answer"><p className="text-sm font-bold text-blue-700">まず、データから言えること</p><h2 className={`${heading} mt-2`}>降水時は低温。でも、雪の多さは別の問い。</h2>
        <p className="mt-4">同時刻で比べると、冬の降水時に八王子のほうが低温だった割合は、比較可能な2つの期間で次のとおりでした。これは<strong>観測できた降水時の記述割合</strong>であり、降雪確率ではありません。</p>
        ...
        <p className="mt-5 text-sm">※東京都心は気象庁の観測地点「東京」（44132）を指します。東京都全域や23区の平均ではありません。観測の均質性が変わるため2つの期間に分け、2014/15年の冬は除外しました。この差をそのまま年代変化とは解釈できません。</p>
      </section>
      ...
      <section className={card} id="mechanism"><h2 className={heading}>01　空から地面まで。雪の運命は途中で変わる。</h2><p className="mt-4">関東の雪で話題になる南岸低気圧。南から水分が運ばれても、地上まで雪のまま届くかは、途中の空気の温度や湿度などに左右されます。地上の気温ひとつで雨と雪を線引きするのは難しいのです。</p>
        ...
        <p className="mt-4">そこで、まず直接比較できる「降水時の気温差」を調べ、雨雪の判別や積雪については別の証拠として考えます。</p>
      </section>
      ...
      <section id="correlation" className="space-y-5"><h2 className={heading}>03　一緒に動くことと、同じ温度は違う。</h2><p>横軸は東京都心、縦軸は八王子。点が等温線より下なら八王子のほうが低温です。寒暖が連動するかは相関、どれだけずれるかは差の分布で読みます。</p>
        {data.primary.segments.map((s, i) => { const fit = regression(s.points.map(p => p.map(Number))); return <div key={s.id} className={card}><h3 className="text-xl font-bold">{periods[i]}</h3><TemperatureScatter points={s.points} /><p className="mt-4">全{s.wet_pairs.toLocaleString()}ペアのPearson相関係数は <strong>r＝{fit.r.toFixed(3)}</strong>。最小二乗の記述的回帰式は、八王子の気温 ≈ {fit.slope.toFixed(3)} × 東京都心の気温 {fit.intercept < 0 ? "−" : "+"} {Math.abs(fit.intercept).toFixed(3)}（℃）です。点は間引かず、重なりを透過表示しています。</p></div>; })}
        <p className="rounded-2xl bg-amber-50 p-6">共通の気象条件や時間帯などが両地点に影響します。相関や回帰式だけで「都心が八王子の気温を変える」とは言えません。この式は対象データの要約であり、降雪予報モデルではありません。<Link href="/toukei/guides/regression-interpretation" className="text-blue-700 underline">回帰の解釈を復習する</Link></p>
      </section>
      ...
      <section id="humidity" className={card}><h2 className={heading}>05　「都心は雨、八王子は雪」を数えられる？</h2><p className="mt-4">ここでデータの壁にぶつかります。八王子の過去の湿度や直接の雨雪観測は、この長期比較にはそろいません。近隣地点の湿度で穴埋めしたり、気温だけを使って雪だったことにしたりすると、問いと答えがずれてしまいます。</p>
        ...
        <p className="mt-4 font-semibold">この12時間を雪になり得た時間とは数えていません。検証済みの雨雪判別方法と十分な対象期間がないため、雪の条件付き確率は算出しません。</p>
      </section>
      ...
      <section id="method" className={card}><h2 className={heading}>方法・再現性・出典</h2><div className="mt-5 space-y-4 text-sm"><p>対象候補は1990/91〜2024/25の35冬（12〜2月）。主解析は気象庁の品質情報8（正常値）のみを採用しました。冬全体の気温・降水量のペア有効率95%以上を条件とし、観測の均質性が冬の途中で変わる2014/15を除いた計34冬を使っています。均質番号は取得単位をまたいで単純比較せず、重複区間の値を照合して接続しています。</p><p>降水イベントは6時間連続の無降水で区切り、不明な降水や観測条件の境界で中断します。前後の区切りを確認できたイベントのうち、降水が3時間以上、気温ペア有効率90%以上を採用しました。仮説1（H1）の支持基準は「5冬・30イベント以上、かつ低温の時間割合と負のイベント平均差の割合がともに60%以上」です。有意差検定の基準ではありません。</p><p>準正常値を含める、区切りを12時間にする、両地点とも降水した時間に絞る、冬の有効率を90%にする感度分析でも、2区間のH1判定は変わりませんでした。</p><p>CSV原本のバイト数・SHA-256・要求した全時間の被覆を確認し、取得境界の3,264レコードを照合しました。主解析とは別のCSV読み取り・イベント集約による170組の照合も通過しています。</p><p>データ版：{data.version} ／ bundle：{snowLock.bytes.toLocaleString()} bytes<br /><span className="break-all">SHA-256：{snowLock.sha256}</span></p><p>AIは構成、解析・実装補助、レビュー補助と導入イラストに使用しました。記事設計にGeminiのレビューを反映しています。運営者が記事プレビューを確認し、2026年9月15日に公開を承認しました。</p></div><ul className="mt-5 space-y-2 text-sm text-blue-700">{sources.map(([label,url]) => <li key={url}><a href={url} className="underline">{label}</a></li>)}</ul></section>
      <section className="rounded-3xl bg-blue-50 p-7">
        ...
        <div className="mt-4 flex flex-wrap gap-5 font-semibold text-blue-700">
          ...
          <Link href="/labs/takao-gear">第5弾：高尾山に山装備は本当に必要か（装備シミュレーター） →</Link>
          <Link href="/labs/takao-weather-shift">第6弾：高尾山頂はなぜガスるのか？（天候急変・見せかけの晴れ） →</Link>
